          ...
          <HeaderMenu label="Layanan" href="/layanan" width="w-[620px]">
            <div className="p-4">
              <div className="flex items-center justify-between border-b border-brand-navy/8 px-2 pb-3">
                <div>
                  <p className="text-[9px] font-semibold uppercase tracking-[0.11em] text-brand-gold-dark">
                    Layanan utama
                  </p>
                  <p className="mt-1 text-xs text-brand-muted">Pilihan layanan yang sering dibutuhkan bisnis.</p>
                </div>
                <Link href="/layanan" className="inline-flex items-center gap-1 text-[11px] font-semibold text-brand-navy">
              ...
              <div className="grid grid-cols-2 gap-x-5 pt-2">
                {menuServices.map((service, index) => (
                  <Link
                    ...
                    className="group/item grid grid-cols-[24px_minmax(0,1fr)_18px] gap-2 border-b border-brand-navy/8 py-3"
                  >
                    <span className="text-[9px] font-semibold text-brand-gold-dark">0{index + 1}</span>
          ...
          <HeaderMenu label="Kategori" href="/layanan" width="w-[500px]">
            <div className="p-4">
              <div className="grid grid-cols-2 gap-x-5">
                {menuCategories.map((category, index) => (
                  <Link
                    ...
                    className="group/item grid grid-cols-[24px_minmax(0,1fr)_18px] gap-2 border-b border-brand-navy/8 py-3 text-xs font-semibold text-brand-navy-dark"
                  >
                    <span className="text-[9px] text-brand-gold-dark">0{index + 1}</span>
              ...
              <div className="mt-2 px-2 pt-2">
                <Link href="/layanan" className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-brand-navy">
                  Lihat semua kategori <ArrowRight className="h-3.5 w-3.5" />
                </Link>
              </div>
            </div>
          </HeaderMenu>
...
function HeaderMenu({
  ...
  return (
    <div className="group relative flex h-full items-center">
      ...
      <div
        className={`pointer-events-none absolute left-1/2 top-[calc(100%-5px)] ${width} -translate-x-1/2 translate-y-2 opacity-0 transition duration-150 group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:opacity-100`}
      >
        <div className="h-3" />
        <div className="border border-brand-navy/12 bg-white">{children}</div>
      </div>
    </div>
  );
}
